...
async function sendMessage() {
    ...
    const {queue} = await channel.assertQueue('email', {exclusive: true, durable: true});

    // Определите тип запроса
    const requestType = 'type2';

    // Отправка запроса
    const request = {
        type: requestType,
        content: 'Запрос типа 2',
        replyTo: queue, // Указываем очередь для получения ответа
    };
    const run = async () => {
        const correlationId = "2_" + Math.random().toString()
